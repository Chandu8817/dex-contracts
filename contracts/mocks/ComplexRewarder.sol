...
contract ComplexRewarder is IRewarder, BoringOwnable{
    ...
    event LogUpdatePool(uint256 indexed pid, uint64 lastRewardBlock, uint256 lpSupply, uint256 accEnergyFiPerShare);
    event LogInit();
    ...
    function onEnergyFiReward (uint256 pid, address _user, address to, uint256, uint256 lpToken) onlyMCV2 lock override external {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][_user];
        uint256 pending;
        if (user.amount > 0) {
            pending =
                (user.amount.mul(pool.accEnergyFiPerShare) / ACC_TOKEN_PRECISION).sub(
                    user.rewardDebt
        ...
        user.amount = lpToken;
        user.rewardDebt = lpToken.mul(pool.accEnergyFiPerShare) / ACC_TOKEN_PRECISION;
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 energyFiReward = blocks.mul(tokenPerBlock).mul(pool.allocPoint) / totalAllocPoint;
            accEnergyFiPerShare = accEnergyFiPerShare.add(energyFiReward.mul(ACC_TOKEN_PRECISION) / lpSupply);
        }
        pending = (user.amount.mul(accEnergyFiPerShare) / ACC_TOKEN_PRECISION).sub(user.rewardDebt).add(user.unpaidRewards);
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                uint256 energyFiReward = blocks.mul(tokenPerBlock).mul(pool.allocPoint) / totalAllocPoint;
                pool.accEnergyFiPerShare = pool.accEnergyFiPerShare.add((energyFiReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
            pool.lastRewardBlock = block.number.to64();
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardBlock, lpSupply, pool.accEnergyFiPerShare);
        }
    }
